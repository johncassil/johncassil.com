...
I'm always creating new variables to filter with.  `dplyr::case_when` is a simple way to do this. For instance, if we wanted a variable to identify if an odometer_reading ends in '0', we can create an `ends_in_0` column.  The general syntax is case_when(condition ~ value, condition ~ value, TRUE ~ value).  TRUE is the catchall "everything else" condition.  With `mutate` we are creating a column called `ends_in_0`, and we are filling it with values from a `case_when` statement.


In our case, if the odometer_reading is divisible by 10, then we want `case_when` to assign `TRUE` to `ends_in_0`, otherwise (if `TRUE`), we want `case_when` to assign `FALSE` to `ends_in_0`.

```{r}
divisible_by_ten <- vins_and_odometers %>% mutate(
   ends_in_0 = case_when(as.integer(odometer_readings) %% 10 == 0 ~ TRUE,
                         TRUE ~ FALSE)
)
divisible_by_ten %>% head()
